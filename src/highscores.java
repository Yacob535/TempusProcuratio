...
public class highscores {
    ...
    public void addHighscore(int score, String name) {
        ...
        for (int k = 0; k < updateScore.size(); k++) {
            if (updateScore.get(k).substring(0, updateScore.get(k).lastIndexOf(" ")).equals(n)&&name.length() <= 16) {
           ...
           // else if(name.length() <= 16){
             //   System.out.println("Please type in a shorter name");
                //When we put this into the actual game, we will reprompt them to put name back in
           // }
        }
        if (ifUsed == true) {
            try {
